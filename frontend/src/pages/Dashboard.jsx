...
import {
    ...
    Loader2, CheckCircle, ArrowRight, Trash2, AlertTriangle, X, ServerCrash
} from "lucide-react";
import CpuArchitecture from "../components/CpuArchitecture";
import "../styles/dashboard.css";

const MOCK_USAGE_LOGS = [
    { id: 1, agent: "Usage", action: "Token and Log Count", tokens: "under progress", time: "cooming soon", status: "under progrees" },
    { id: 2, agent: "RAG+", action: "Better Accuracy", tokens: "testing", time: "cooming soon", status: "under progrees" },
    { id: 3, agent: "PDF", action: "Download PDF", tokens: "ongoing", time: "cooming soon", status: "under progrees" },
];
...
export default function Dashboard() {
    ...
    useEffect(() => {
        ...
        if (savedStatus === "running") {
            if (!isEnvironmentAlive) {
                // CASE 1: Browser Refresh / New Tab -> Kill Process
                ...
                // CASE 2: Navigated back -> Keep Running
                setGenerationStatus("running");
                startProgressLoop();
            }
        }
        else if (location.state?.resetForm) {
            // CASE 3: Explicit reset via navigation
            ...
            // Sync ID just in case state is empty but storage has it
    ...
    const generateCourse = async (e) => {
        ...
        try {
            ...
            if (res.data.success) {
                ...
                // Checks for 'course_id', 'id', or 'courseId' to be safe
                const rawId = res.data.course_id || res.data.id || res.data.courseId;
                const newId = rawId ? parseInt(rawId, 10) : null;

                if (!newId) {
                    throw new Error("Server responded with success but no Course ID.");
                }
    ...
    const openCourse = (courseId) => {
        // Fallback: If passed ID is null, try to find it in localStorage
    ...
    const handleToastClick = () => {
        if (generationStatus === "completed") {
            // Use openCourse which handles null checks
        ...
        <div className="app-container">
            ...
            {errorModal.show && (
                <div className="dashboard-modal-overlay">
                    <div className="dashboard-modal dashboard-modal-error">
                        ...
                        <button
                            onClick={() => setErrorModal({ show: false, message: "" })}
                            className="btn dashboard-modal-primary-btn"
                        >
                            Dismiss
                        </button>
                    </div>
                    ...
                    <div className="dashboard-modal dashboard-modal-delete">
                        ...
                        <div className="dashboard-modal-actions">
                            <button
                                ...
                                className="btn dashboard-modal-btn dashboard-modal-btn-cancel"
                            >
                                Cancel
                            </button>
                            <button
                                onClick={executeDelete}
                                className="btn dashboard-modal-btn dashboard-modal-btn-danger"
                            >
                                Delete
                            </button>
                        </div>
                    ...
                    {generationStatus === "running" ? (
                        <>
                            ...
                            <div>
                                ...
                                <span className="dashboard-toast-subtitle">
                                    Building "{form.topic || "Course"}" ({Math.round(progress)}%)
                                </span>
                            </div>
                        </>
                    ) : (
                        <>
                            <div className="dashboard-toast-icon-success">
                                <CheckCircle size={18} />
                            </div>
                            <div>
                                <span className="dashboard-toast-title">Course Ready!</span>
                                <span className="dashboard-toast-link">
            ...
            <aside className="sidebar">
                <div className="user-profile">
                    <img
                        src="https://ui-avatars.com/api/?name=Student&background=333&color=fff"
                        alt="Profile"
                        className="avatar"
                    />
            ...
            <main className="main-content-area">
                        ...
                        <div className="dashboard-usage-list">
                            {MOCK_USAGE_LOGS.map((log) => (
                                <div key={log.id} className="dashboard-usage-row">
                                    ...
                                    <div className="dashboard-usage-time">
                                        <Clock size={14} /> {log.time}
                                    </div>
                                    <div
                                        className={`dashboard-usage-tokens ${
                                            log.status === "success"
                                                ? "dashboard-usage-tokens--success"
                                                : "dashboard-usage-tokens--warning"
                                        }`}
                                    >
                    ...
                    <div className="fade-in dashboard-generate-root">
                            ...
                            <div className="dashboard-generate-form-shell">
                                <header className="dashboard-header dashboard-generate-header">
                                    <div>
                                        ...
                                        <h1 className="dashboard-generate-title">
                                            Orchestrate New Course
                                        </h1>
                                        <p className="dashboard-generate-subtitle">
                                            Define parameters for your personal AI swarm.
                                        </p>
                                    </div>
                                </header>
                                <div className="glass-card dashboard-generate-card">
                                    <form onSubmit={generateCourse}>
                                        ...
                                        <button
                                            type="submit"
                                            className="btn btn-primary dashboard-generate-submit"
                                        >
                                            Initialize Agents &amp; Build Course
                                        </button>
                                    </form>
                                </div>
                            </div>
                        ...
                        {generationStatus === "running" && (
                            <div className="dashboard-running-view">
                                ...
                                <p className="dashboard-running-text">
                                    Agents are researching {form.topic || "content"}...
                                </p>
                        ...
                        {generationStatus === "completed" && (
                            <div className="dashboard-completed-view">
                                ...
                                <p className="dashboard-completed-text">
                                    Your personalized course has been generated.
                                </p>
                                <div className="dashboard-completed-actions">
                                    <button
                                        className="btn btn-primary dashboard-completed-primary-btn"
                                        onClick={() => openCourse(generatedCourseId)}
                                    >
                                        Start Learning Now
                                    </button>
                                    <button
                ...
                {activeTab === "courses" && (
                    <div className="fade-in">
                        <header className="dashboard-header">
                            <div>
                                <h1 className="dashboard-page-title">Active Courses</h1>
                            </div>
                        </header>
                        <div className="bento-grid">
                            <div
                                className="bento-card create-card dashboard-create-card"
                                onClick={() => setActiveTab("generate")}
                            >
                                <div className="dashboard-create-card-icon">
                                    <PlusCircle size={24} />
                                </div>
                                <span className="dashboard-create-card-label">
                                    Create New Course
                                </span>
                            </div>
                            ...
                            {courses.map((course) => (
                                ...
                                >
                                    {/* NEW COURSE STICKER */}
                                    {course.id === newlyCreatedId && (
                                        <div className="dashboard-course-new-badge">
                                            NEW
                                        </div>
                                    )}

                                    <button
                                        onClick={(e) => confirmDeleteRequest(e, course.id)}
                                        className="dashboard-course-delete-btn"
                                    >
                                        <Trash2 size={16} />
                                    </button>
