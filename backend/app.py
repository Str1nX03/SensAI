...
# --- UPDATED IMPORTS FOR ROOT LOCATION ---
# Since app.py is in root, we import from 'backend.src...'
...
# Update DB path so it doesn't clutter root (optional, or keep as 'sqlite:///users.db')
...
# --- CORS SETUP ---
# Allow React (Port 5173) to talk to Flask
...
# 1. GET COURSES (Matches Dashboard.jsx)
...
# 2. GENERATE COURSE (Matches Dashboard.jsx)
...
async def generate_course():
    ...
    # Reuse existing course if found
    ...
    try:
        ...
        course = Course(
            ...
            links=json.dumps(out.get("raw_study_links", [])),
        ...
        return jsonify({"success": False, "message": "Agent generation failed"}), 500


# 3. GET SINGLE COURSE (Matches Product.jsx)
